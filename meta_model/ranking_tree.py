...
class DecisionTree(ABC):
    ...
    def fit(self, Xin: np.array, Yin: np.array) -> DecisionTree:
        """
...
class RankingTree(DecisionTree):
    ...
    def __ndcg(self, D: dict) -> float:
        ...
        res = pairwise_distances(
            D["Y"], np.array([y_pred,]),
            metric=lambda x,y: _ndcg(x, y, p=5),
            n_jobs=self.n_jobs
        )
        return np.mean(res)
...
if __name__ == "__main__":
    import time
    X = np.random.uniform(size=(100, 61))
    Y = np.random.uniform(size=(100, 120))
    clf = RankingTree(rank_sim='ndcg', min_samples_split=5)
    print("fitting...", end="")
    start = time.time()
    clf = clf.fit(X, Y)
    end = time.time()
    print(f"END. {end - start} s.")
    print(clf.predict(np.random.uniform(size=(5, 61))))