...
from LSHkRepresentatives.ClusteringAlgorithm import ClusteringAlgorithm
from sklearn.metrics.cluster import adjusted_rand_score
from sklearn.metrics.cluster import normalized_mutual_info_score
from sklearn.metrics.cluster import adjusted_mutual_info_score
from sklearn.metrics.cluster import homogeneity_score
import random
from LSHkRepresentatives.LSH import LSH 
import multiprocessing as mp
import math
from collections import defaultdict
from LSHkRepresentatives.LSHkRepresentatives import CategoricalDatasetNormalization

        
class LSHkPrototypes(ClusteringAlgorithm):

    def SetupLSH(self, hbits=-1, k=-1):
        self.CheckNormalizedData()
        start = timeit.default_timer()
        self.lsh = LSH(self.X_CATE, self.y, measure=self.measure, hbits=hbits, k=self.k)
        self.lsh.DoHash()
    ...
    def Distance_CATE(self,representative, point):
        ...
        for i in range (self.d_CATE):
            d += sum([representative[i][u]*self.measure.distMatrix[i][u][point[i]] for u in range(self.D_CATE[i])])
        return d
        
    def Distance_NUM(self,center, point):
        return self.num_metric.dist(center, point)
...
if __name__ == "__main__":
    ...
    X_NUM = data["Xnum"]
    X_CATE = data["Xcat"]
    X = np.c_[X_NUM, X_CATE]
    attributeMasks = [1 for _ in range(X_NUM.shape[1])] + [0 for _ in range(X_CATE.shape[1])]
    y = data["y"]
    ...
    start = timeit.default_timer()
    labels = LSHkPrototypes(n_clusters, num_metric=num_metric, cat_metric=cat_metric).fit(X, attributeMasks=attributeMasks, dataset_name=data["id"])
