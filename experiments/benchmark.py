...
from kprototypes import KPrototypes as KP
...
def run_kprototypes(Xnum, Xcat, n_clusters, num_metric, cat_metric, gamma, n_init=5):
    final_clusters = None
    best_loss = np.inf
    for random_state in range(n_init):
        try:
...
def optimize_kprototypes(Xnum, Xcat, y, n_clusters, num_metric, cat_metric, eval_metric):
    def black_box_function(gamma):
        clusters = run_kprototypes(Xnum, Xcat, n_clusters, num_metric, cat_metric, gamma)
        if clusters is None : return -1
        return get_score(y, clusters, eval_metric=eval_metric)
...
def optimize_with_data(algorithm, Xnum, Xcat, y, n_clusters, num_metric, cat_metric, eval_metric):
    if algorithm == "kprototypes":
        return optimize_kprototypes(Xnum, Xcat, y, n_clusters, num_metric, cat_metric, eval_metric)

def benchmark(id_, data, results_file, result=None, algorithm="ac", eval_metric = "acc"):
    start = time.time()
    Xnum, Xcat, y = data
    ...
    def run_with_data(Xnum, Xcat, num_metric, cat_metric):
        res = {}
        weights, score = optimize_with_data(algorithm, Xnum, Xcat, y, n_clusters, num_metric, cat_metric, eval_metric)
        res["params"] = weights
        res["params"]["n_clusters"] = n_clusters
        res["score"] = score
        return res
    ...
    if algorithm in ["kprototypes"]:
        ...
        for num_metric in base_metrics.get_available_metrics(data_type="numeric"):
            if base_metrics.get_metric(num_metric).is_valid_data(Xnum):
                try:
                    ...
                    print(f"Error: While computing pairwise distance | dataset: {id_}, metric: {num_metric}")
        ...
        for cat_metric in base_metrics.get_available_metrics(data_type="categorical"):
            if base_metrics.get_metric(cat_metric).is_valid_data(Xcat):
                try:
                    ...
                    print(f"Error: While computing pairwise distance | dataset: {id_}, metric: {cat_metric}")
                    print(f"Traceback: {traceback.format_exc()}")
        ...
        for bin_metric in base_metrics.get_available_metrics(data_type="binary"):
            if base_metrics.get_metric(bin_metric).is_valid_data(Xdummy):
                try:
                    fitted_bin_metric = base_metrics.get_metric(bin_metric).fit(Xdummy)
                except:
                    fitted_bin_metric = None
                    print(f"Error: While computing pairwise distance | dataset: {id_}, metric: {bin_metric}")
                    print(f"Traceback: {traceback.format_exc()}")
            ...
            if fitted_num_metric is not None:
                for cat_metric, fitted_cat_metric in all_fitted_cat_metric.items():
                    if fitted_cat_metric is not None and f"{num_metric}_{cat_metric}" not in result:
                        result[f"{num_metric}_{cat_metric}"] = run_with_data(Xnum, Xcat, fitted_num_metric, fitted_cat_metric)
                for bin_metric, fitted_bin_metric in all_fitted_bin_metric.items():
                    if fitted_bin_metric is not None and f"{num_metric}_{bin_metric}" not in result:
                        result[f"{num_metric}_{bin_metric}"] = run_with_data(Xnum, Xdummy, fitted_num_metric, fitted_bin_metric)
    else:
    ...
    if len(result) > 0:
        duration = end - start
        time_file = os.path.join(os.path.dirname(results_file), f"benchmark_duration_{args.algorithm}_{args.evalmetric}_augmented_grid2.json")
...
if __name__=="__main__":
    ...
    results = {}
    results_file = os.path.join(args.outdir, f"benchmark_results_{args.algorithm}_{args.evalmetric}_augmented_grid2.json")
    if os.path.isfile(results_file):
        with open(results_file, "r", encoding="utf-8") as f:
            results = json.load(f)
    ...
    list_ids = np.unique([id_.split("_")[0] for id_ in datasets_info])

    n_datasets = len(list_ids)
    print(f"{n_datasets} data sets to load")
    ...
    print()
    print(f"{n_loaded}/{n_datasets} data sets loaded (-> {len(datasets)} after augmentation)")
    
    print()
    print("BENCHMARKING...")
    list_ret = Parallel(n_jobs=args.jobs, verbose=60)(
        delayed(benchmark)(
            augmented_id, data, results_file, 
            result=results[augmented_id] if augmented_id in results else None, 
